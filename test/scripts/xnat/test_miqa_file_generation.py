...
def test_json_convert_check_new_sessions_df(file_prefix):
    ...
    assert not json_df.empty, "Was not able to translate json file"
    ...
    if False:
        print(json_df[['xnat_experiment_id','scan_id', 'scan_type', 'decision','scan_note']])
        print("====")
        print(csv_df[['xnat_experiment_id','scan_id', 'scan_type','decision','scan_note']])
...
def test_write_to_json_(file_name):
    ...
    #
    # Test a: check that you can turn an empty DF into miqa dictionary 
    #
    df = pd.DataFrame(columns=rows_cols[0])    
