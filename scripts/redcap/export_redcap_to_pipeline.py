...
# "Safe" CSV export - this will catch IO erors from trying to write to a file that is currently being read and will retry a number of times before giving up
# This function will also confirm whether the newly created file is different from an already existing file of the same name. Only changed files will be updated.
    ...
    for export_name in export_list:
        ...
        if len( record ) == 1:
            ...
            for field in record.columns:
                ...
                # If this is a radio or dropdown field (except "FORM_[missing_]why"), add a separate column for the coded label
                if field in code_to_label_dict.keys() and not re.match( '.*_why$', field ):
            ...
            safe_csv_export( record, os.path.join( measures_dir, export_name + '.csv' ), verbose=verbose )


# Filter potentially confidential fields out of given list, based on project metadata
def filter_out_confidential( field_list, metadata_dict ):
    filtered_list = []
    for field_name in field_list:
	try:
	    (field_type,field_validation,field_label,text_val_min,text_val_max,choices) = metadata_dict[ re.sub( '___.*', '', field_name ) ]
            if  (field_type != 'text' and field_type != 'notes') or (field_validation in [ 'number', 'integer', 'time' ]):
                filtered_list.append( field_name )
            else:
                print "WARNING: field '%s' is of type '%s' with validation '%s' - excluding as potentially confidential." % ( field_name, field_type, field_validation )
        except:
...
def filter_all_forms( redcap_metadata ):
    ...
    # Filter each form
    for export_name in export_forms.keys():
        export_forms[export_name] = filter_out_confidential( export_forms[export_name], metadata_dict )
...
def create_datadicts_general( datadict_dir, datadict_base_file, export_forms_list, variable_list):
    redcap_datadict_columns = [ "Variable / Field Name", "Form Name", "Section Header", "Field Type", "Field Label", "Choices, Calculations, OR Slider Labels", "Field Note", "Text Validation Type OR Show Slider Number", "Text Validation Min", "Text Validation Max","Identifier?", "Branching Logic (Show field only if...)", "Required Field?", "Custom Alignment", "Question Number (surveys only)", "Matrix Group Name", "Matrix Ranking?"]
...
# defining entry_list only makes sense if export_forms_list only consists of one entry !
...
#
# Initialization - figure out which instruments we're supposed to export, and which fields
#
import_forms = dict()
export_forms = dict()
export_names = dict()
