...
def find_phantom_scan_24h(prj, experiment_label, seid, experiment_last_modified, phantom_id, edate, etime, scanner, args, email):
...
def check_experiment(session, args, email, eid, experiment):
    ...
    try:
        prj, sid, seid, experiment_label, edate, etime, scanner = expUtil.mget(['project', 'subject_ID', 'ID', 'label', 'date', 'time', 'scanner'])
    except:
    ...
    subject_label_match = re.match(subject_id_pattern_nophantom, session.xnat_get_subject_attribute(prj,sid,'label')[0])
    if subject_label_match:
        ...
        try:
            ...
            [phantom_id, issue_url] = session.xnat_get_subject_attribute(prj,phantom_label,'ID')
            if not phantom_id :
                slog.info(eid,"Could not get phantom id for phantom " + phantom_label, 
                          project_id=prj,
            ...
            phantom_scanners = [ifc.select.experiments[eid].get('scanner') for eid in eids]
            if args.verbose:
            ...
            if scanner not in phantom_scanners and len(phantom_scans) != 0:
                ...
                find_phantom_scan_24h(prj, experiment_label, seid, experiment_last_modified, phantom_id, edate, etime, scanner, args, email)
...
if __name__ == "__main__": 
    ...
    try:
        ...
        if args.verbose:
            print("Last checked on: {0}".format(time.strftime(XNAT_DATE_FORMAT,
                                                              date_last_checked)))
    except:
        pass
