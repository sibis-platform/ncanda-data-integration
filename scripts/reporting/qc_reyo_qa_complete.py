...
Reyo_QA_Completed
======================
Generates a list of subjects whose QA for Reyo has not been completed
"""
...
fields = ['study_id', 'redcap_event_name','exclude', 'visit_ignore',
          'visit_date','np_reyo_missing','np_reyo_qc(completed)'];
...
def value_check(idx,row):
	"""
	Checks to see if an QA for Reyo has not been completed
	"""
	# visit_ignore____yes with value 0 is not ignored
	error = dict()
	if math.isnan(row.get('exclude')):
		if row.get('visit_ignore___yes') != 1:
			# reyo is not missing if saliva_sample_missing if value zero
			if row.get('np_reyo_missing') != 1:
				if row.get('np_reyo_qc(completed)') != 0:
					error = dict(subject_site_id = idx[0],
								visit_date = row.get('visit_date'),
								event_name = idx[1],
								error = 'ERROR: Reyo upload needing QA'
								)
	return error
