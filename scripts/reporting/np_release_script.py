...
fields = ['site', 'sex', 'visit_age', 'exceed_or_mrianomaly',
          'exceeds_bl_drinking', 'cddr_past_month_binge',
          'cddr_past_year_binge', 'binge_groups_1', 'binge_groups_month',
          'hispanic', 'race', 'race_label', 'mri_analysisanomalies', 'hi_ed',
...
def main(args=None):
    ...
    binge_df = final_df['cddr_past_year_binge']
    final_df['binge_groups_1'] = binge_df.apply(replace_binge_groups_1)
    groups_df = final_df['cddr_past_month_binge']
    final_df['binge_groups_month'] = groups_df.apply(replace_binge_groups_month)
    ...
    final_df.to_csv(os.path.join(args.outputdir,
                                 'np_release_{}.csv'.format(today)))
